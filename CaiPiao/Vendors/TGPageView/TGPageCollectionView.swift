...
//    fileprivate var collectionView : UICollectionView?
    fileprivate var collectionView : TGCollectionView!//用！ 则后面使用到不用写写？  ->  用！意思是这行代码的作者对此保证：若有用到此collectionView,则一定有值  ->  若用？ 则后面的代码一直要用？，系统不会强行解包，而是进行可选链形式操作，如果没有值，则不往下操作  ->  如果使用的是！，发现没有值，则会崩  ->  所以你有把握用! 没有把握用？
    ...
    private func setupUI(){
        ...
        let contentView = TGCollectionView(frame: contentFrame, collectionViewLayout: layout)
